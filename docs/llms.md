...
- **Agent Runtime** (`src/agents/`): Pi SDK integration (Pi packages pinned at `^0.80.6` under the `@earendil-works` scope; runtime helpers that moved out of the `@earendil-works/pi-ai` root in 0.80.0 — `getEnvApiKey`, `completeSimple` — are imported from `@earendil-works/pi-ai/compat`; built-in coding tools are enabled by name via `createAgentSession({ tools: ["read", "bash", "edit", "write"] })`), session management, sandbox container mount/argument helpers in `src/agents/container.ts`, and the Docker-backed container adapter in `src/sdk/container/adapter.ts`
  - `src/agents/run-lifecycle.ts` owns gateway session run state transitions behind `SessionRunLifecycle`: active streaming/abort state, adapter handles, queue vs interrupt joins, pending follow-up messages, turn buffering, history event emission, and final turn flushing. `runAgent()` still resolves agents/sessions, handles `/abort` and `/think`, selects adapters, invokes SDKs, and drains non-native queued runs. Explicit `/abort` and `/stop` pause an active durable task; ordinary follow-ups to active durable work are forced to queue and cannot alter its lifecycle.
- **Scheduler** (`packages/extensions/scheduler/`): Interval/daily job execution. Scheduled fires default to unique sessions (`scheduler:<jobId>:<runId>`) unless `payload.sessionId` explicitly overrides the target. Scheduler job files may include optional top-level `model: { provider, model }`; when present, scheduled runs use that provider/model instead of the agent default. A job's `payload` shape (not a `kind` field) decides its behavior: `message` alone is a plain agent job; `script` + `noAgent: true` is script-only (a subprocess whose exit code — never an LLM self-report — decides `ok`/`error`); `script` + `message` is a wakeAgent gate (the script runs first and its final stdout JSON line decides whether the agent runs at all, defaulting to waking on anything unparseable). Opt-in `payload.quietOutput: true` skips the run's output file for uneventful script-only/silent-tick runs only; errors and woke-agent runs always write it. When enabled, scheduler injects self-only agent tools (`scheduler.list_jobs`, `scheduler.create_job`, `scheduler.update_job`, `scheduler.delete_job`, `scheduler.get_latest_output`); `create_job`/`update_job` accept `script`/`noAgent`/`quietOutput` alongside `message` (same shape rules as the payload schema) and an optional `timeoutMs` (per-run timeout, default 30 minutes) — they do not yet expose `deliver`. A job-level (sibling of `payload`) optional `deliver: DeliverTarget[]` list pushes a resolved run's result to comm-channel sinks (`discord`/`slack`/`telegram`) at the runtime level, never as an LLM tool call: the agent's response (or script stdout), truncated at 4000 chars, except a silent tick or empty-stdout script run delivers nothing and any errored run always delivers an alert; a missing sink or a throwing sink is recorded as a per-target warning, never a run failure. The `yoplai scheduler` CLI has not been extended for `script`/`noAgent`/`quietOutput`/`deliver` yet. Gateway hot reload polls config, agent YAML, and agent cron files every 5 seconds. See `packages/extensions/scheduler/README.md`.
...
- Agent chat shows estimated context usage under the input, turns the indicator red at 75%+, and runs context compaction before the next send at 80%+. `/compact` triggers the same compaction manually. Compaction asks the same agent/model to summarize older context, then rewrites Yoplai canonical history and the Pi runtime session to a hidden compacted summary plus the last 8 user/assistant turns. `compactAgentSession()` is single-flight per (userId, agentId, sessionId): concurrent requests share one promise so the history read, summarization, canonical rewrite, and runtime seed happen once; the entry is cleared on both success and failure.
...
- User↔team membership is a many-to-many relation backed by a `team_members` table (`teamId`, `userId`, `addedBy`, `addedAt`; composite PK `(teamId, userId)`; FKs to `teams`/`user` with cascade delete, plus an index on `userId`). A user may belong to many teams and a team may hold many users. The membership deep module (`membership.ts`) exposes idempotent add (`ON CONFLICT DO NOTHING`), remove, `listTeamsForUser`, `listUsersForTeam`, and `usersOnlyInTeam` (the teamless-set source used by `deleteTeam`). Admins add/remove members via `POST /api/admin/teams/:teamId/members` (body `{ userId }`, 404 for a missing team/user, idempotent 200 on re-add) and `DELETE /api/admin/teams/:teamId/members/:userId`; both are guarded by `requireAdmin`. Any authenticated user reads a team's members via `GET /api/teams/:teamId/members` (global visibility). The Teams page adds a per-team members view: everyone can open it to see members, admins get an add-user picker and per-member remove controls. Admin edits (All users toggle, Add, Remove) are staged locally in the modal and only reach the server on Save (a single `PUT`-style `setTeamMembers` call), with Save/Cancel appearing only once the draft differs from server state — an accidental toggle or remove can't wipe a hand-built roster before the admin confirms. Switching a team to All users leaves its prior explicit `team_members` rows in place (latent — inert under the `allUsers`-gated read queries) instead of deleting them; `listSavedMemberProfilesForTeam` exposes that latent roster, and the members response includes it as `savedMembers` while `allUsers` is true, so unchecking All users in the modal restores the saved roster into the draft rather than the expanded all-user list or an empty set.
- Startup flow:
...
- Sandbox container manager helpers in `apps/gateway/src/agents/container.ts` build Docker bind mounts, shadow workspace `.env` with `/dev/null`, validate custom mounts against the sandbox allowlist/blocklist, build `docker run -i --rm` args, and provide Docker network/orphan cleanup helpers. `apps/gateway/src/sdk/container/adapter.ts` composes focused internal modules, spawns ephemeral Docker containers, writes `ContainerInput` to stdin, parses `---YOPLAI_OUTPUT_START---`/`---YOPLAI_OUTPUT_END---` output, queues follow-ups through `$YOPLAI_HOME/ipc/<agentId>/<sessionId>-<runId>/input/*.json`, and stops/kills containers on abort or timeout.
- Container IPC is scoped per run, not per agent. `getRunIpcDir()` derives `$YOPLAI_HOME/ipc/<agentId>/<sessionId>-<runId>` and `buildContainerLaunchSpec()` mints the `runId`, so concurrent containers for one agent cannot consume each other's queued follow-ups or abort sentinels. Queued messages are `ContainerDeliveryEnvelope`s stamped with `agentId`/`sessionId`/`runId`; the runner's `resolveIpcDelivery()` drops anything addressed elsewhere (`agent_mismatch`/`session_mismatch`/`run_mismatch`) and falls back to agent+session matching when `ContainerInput.runId` is absent (containers from an older image). The gateway removes a run's namespace when the run settles, refuses follow-ups and aborts against a settled handle (`RunSettledError`, re-buffered for the next run by `SessionRunLifecycle.handleJoin`), and `cleanupOrphanIpcNamespaces()` sweeps namespaces orphaned by a crash alongside `cleanupOrphanContainers()`.
- `yoplai-agent:latest` is content-hashed from the agent-runner Docker build context and rebuilt at startup when those inputs drift. Operator-supplied sandbox images are never hash-checked or rebuilt.
...
**Script jobs (`packages/extensions/scheduler/src/script.ts`).** `SchedulePayloadSchema` (`packages/shared/src/types.ts`) makes `message` optional and adds `script` (workspace-relative path), `noAgent`, and `quietOutput`, with a `superRefine` validation matrix shared by file load, the HTTP API, and agent tools (blank `message`/`script` rejected; `noAgent` requires `script` and rejects `message`; `script` without `noAgent` requires `message`; `quietOutput` requires `script`; absolute paths and `..` segments rejected). At fire time `resolveScriptPath` re-checks containment against the workspace directory's **realpath**, catching symlink escapes the string check can't see, and errors when the script doesn't exist. `runScript` picks the interpreter from the extension only — `.sh`/`.bash` run under `bash <path>`, anything else must already be executable (`chmod +x`) or the run fails with `script <path> is not executable` — pipes and caps stdout/stderr at 64 KiB each while streaming (`[output truncated]` marker past the cap), and on timeout kills the whole POSIX process group (`SIGKILL` via `process.kill(-pid, ...)`) so a shell script's foreground child dies with it. A non-zero exit throws `script failed (exit N)` (or `script failed (exit signal)` when signal-killed); the service parses `N` back out of that message into `state.lastExitCode`. `parseWakeAgent` reads the script's last non-empty stdout line as JSON: `{"wakeAgent": false}` is a silent tick (agent never runs); `{"wakeAgent": true, "context": {...}}` runs the agent with `message + "\n\nGate context:\n" + JSON.stringify(context)`; a missing key, unparseable line, or no stdout all default to waking. `noAgent: true` jobs skip this parse entirely — only the exit code decides `ok`/`error`. `state` gains `lastExitCode?: number` and `lastRunKind?: "script_only" | "silent_tick" | "woke_agent"`, both runtime-only (stripped on save, like the rest of `state`). Output-file `status_label`/`**Status:**` is one of `ok`, `ok (silent tick)`, `woke agent`, `script failed (exit N)`, `script failed (exit signal)`, or the generic `error` (agent failures, and script timeouts, which are not classified as `script failed (exit N)`). `payload.quietOutput: true` skips `writeCronRunOutput` only for an uneventful run (script-only with empty trimmed stdout, or a silent tick) — errors and non-empty output always write, and runtime state updates every tick regardless. Script-only and silent-tick runs never open a session, so their output file omits `session_id` and `## Prompt`. Full copy-pasteable job examples and a sample wakeAgent gate script live in `packages/extensions/scheduler/README.md`.

**Delivery (`packages/extensions/scheduler/src/deliver.ts`).** `DeliverTargetSchema` (`packages/shared/src/types.ts`) validates a job-level, `payload`-sibling `deliver: DeliverTarget[]` list — each entry `{ target, channel? | user? }`, non-empty `target`, exactly one of `channel`/`user` non-empty when supplied — accepted by `ScheduleJobFileSchema`, `CreateScheduleRequestSchema`, and `UpdateScheduleRequestSchema` (not yet by the `scheduler.create_job`/`update_job` agent tools, which have no `deliver` parameter). Comm extensions register a `DeliverySink` under their extension id (`discord`/`slack`/`telegram`) via the host-owned registry on `ExtensionContext` (`registerDeliverySink`/`getDeliverySink`, `apps/gateway/src/extensions/context.ts`), sharing their existing `*.send_message` send internals; a second registration under the same id replaces the first and logs a warning, and each extension unregisters in `stop()`. After every run resolves — including one whose `quietOutput` skipped the output file — `deliverRunResult` computes `deliveryText` (agent job: the runner's response; script-only: trimmed stdout, empty means no delivery; silent tick: no delivery; woke agent: the agent's response; any errored run: always an alert naming the job and the error, truncated at `MAX_DELIVERY_CHARS` = 4000 chars with a `[truncated]` marker) and fans it out to every configured target. A target with no registered sink, or whose sink throws, is recorded as a `DeliveryOutcome` warning (`state.lastDelivery`) and in the output file's `## Delivery` section — it never flips the run's `ok`/`error` status and never blocks the remaining targets. `packages/extensions/irc/` has no proactive send path, so it has no sink and any `deliver` entry targeting it is recorded as an unregistered-target warning. Full copy-pasteable examples live in `packages/extensions/scheduler/README.md`.

CLI: `yoplai scheduler add <agent-id> --cron <expr> --tz <iana> -m <message>`, `list [--agent <id>]`, `update <agent-id> <job-id>`, `rm <agent-id> <job-id>`, and `tail <agent-id> <job-id>`. API breaking changes: update/delete/tail paths are agent-scoped (`/api/schedules/:agentId/:id`), and schedule payloads use cron+required `tz` instead of old interval/daily variants. CLI registration lives in `packages/extensions/scheduler/src/cli/`; wired into the gateway CLI at `apps/gateway/src/cli/index.ts`. The CLI has not been extended for `script`/`noAgent`/`quietOutput`/`deliver` yet (unlike the agent tools, and `deliver` isn't in the agent tools either); script, gated, and delivering jobs made from the CLI's side go through `cron/jobs.json` or the HTTP API, both of which validate through the same schemas.
