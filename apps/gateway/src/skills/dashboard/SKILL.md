...
1. Inspect existing `dashboards/`, `data/`, and `data/migrations/`. Update existing files in place; do not rewrite a dashboard that only needs a refinement.
...
4. Put each page at `dashboards/<slug>.html`. Use live SQL blocks and the Canvas runtime exactly as described in [canvas.md](references/canvas.md). Use the Dashboard Kit and ECharts patterns in [kit.md](references/kit.md).
...
8. Call `dashboard_link` after every create or update. Reply on the current channel—web, Slack, Discord, or elsewhere—with its absolute clickable URL. Never say only “it is in your dashboards tab.”
...
```sh
mkdir -p dashboards data/migrations
cp <skill-dir>/templates/hr/dashboards/pto-me.html dashboards/
